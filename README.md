...
The BOT Testnet protocol and real-evidence application path are implemented. The production UI accepts no preset payment scenarios, fixed terms, fixed periods, demo claim IDs, or verifier-generated evidence. Each asset commits issuer-entered terms; each claim commits the canonical hash of an externally signed evidence bundle; and the verifier checks that bundle against live BOT Chain claim state, the registered terms and policy hashes, the configured evidence-source address, source signature, freshness, payer reference, amount, and payment date. Golden fixtures remain isolated to automated tests and historical acceptance artifacts. Forty-five automated tests prove the signed-evidence-to-holder protocol, challenge, slashing, refund, replay, and fail-closed invariants. Both the native Next.js and Cloudflare-compatible Sites builds pass.
