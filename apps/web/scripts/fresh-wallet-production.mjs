...
await confirm("createAsset", await freshClient.writeContract({ address: manifest.contracts.assetFactory, abi: factoryAbi, functionName: "createAsset", args: [assetId, "VeriFi Fresh Wallet Asset", "vFRESH", policyHash, termsHash, [fresh.address, fresh.address], [parseUnits("60", 18), parseUnits("40", 18)]] }));
...
const message = ["VeriFi BOT Testnet attestation request", `Claim: ${claimId}`, "Chain: 968", "Purpose: authorize the bonded verifier to inspect this claim's committed sandbox evidence."].join("\n");
