...
export default function Home() {
  ...
  const [active, setActive] = useState<Action>("claim");
  ...
  async function submitClaim(event: FormEvent<HTMLFormElement>) {
    ...
    if (!contracts.vault || !contracts.settlement) return;
    const form = new FormData(event.currentTarget);
    if (active === "asset" && contracts.assetFactory) {
      const holderA = String(form.get("holderA"));
      const holderB = String(form.get("holderB"));
      if (!isAddress(holderA) || !isAddress(holderB)) {
        setStatus("Both initial holder addresses must be valid EVM addresses.");
        return;
      }
      ...
      setStatus("Asset creation submitted. The factory will register it and allocate snapshot-enabled shares.");
    }
    ...
    setStatus("Yield claim submitted. The verifier agent will inspect its committed evidence.");
  }
  ...
  async function runSimple(event: FormEvent<HTMLFormElement>) {
    ...
    if (active === "challenge" && contracts.attestation) {
      ...
      setStatus("Challenge submitted with a 1 BOT bond for resolver review.");
    }
            ...
            {active === "claim" ? (
              <form onSubmit={(event) => void safelyRun(submitClaim, event)}>
                ...
                <div className="field-grid">
                  <label>Asset ID<input name="assetId" defaultValue="asset:verifi-solar-001" required /></label>
              ...
              <form onSubmit={(event) => void safelyRun(runSimple, event)}>
                ...
                {active === "asset" && <>
                  <div className="field-grid"><label>Asset ID<input name="newAssetId" defaultValue="asset:issuer-solar-002" required /></label><label>Committed terms<input value="2,000 USDT · due Aug 1 · exact payer" readOnly /></label></div>
                  ...
                  <div className="field-grid"><label>Holder A<input name="holderA" defaultValue={address} placeholder="0x…" required /></label><label>Holder A shares<input name="holderAShares" type="number" defaultValue="60" min="0.000001" step="0.000001" required /></label></div>
                  <div className="field-grid"><label>Holder B<input name="holderB" defaultValue={address} placeholder="0x…" required /></label><label>Holder B shares<input name="holderBShares" type="number" defaultValue="40" min="0.000001" step="0.000001" required /></label></div>
                ...
                {active === "challenge" && <><label>Attestation ID<input name="attestationId" placeholder="0x…" required /></label><label>Counter-evidence reference<input name="counterEvidence" placeholder="IPFS CID, document hash, or reference" required /></label><p className="bond-note"><LockKeyhole size={15} /> Requires a 1 BOT challenge bond.</p></>}
